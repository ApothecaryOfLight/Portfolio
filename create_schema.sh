...
echo "CREATE TABLE blog_posts( title MEDIUMTEXT, timestamp TIMESTAMP, body LONGTEXT, post_id INT, PRIMARY KEY(post_id) );" >> "create_schema.sql"
echo "CREATE TABLE blog_series( series_title MEDIUMTEXT, timestamp TIMESTAMP, post_id INT, FOREIGN KEY (post_id) REFERENCES blog_posts(post_id), number_in_series INT, series_id INT );" >> "create_schema.sql"
