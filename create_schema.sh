...
echo "CREATE TABLE blog_images( image_id INT, PRIMARY KEY(image_id), image_data MEDIUMTEXT, alt_text TEXT, optional_link TEXT, post_id, FOREIGN KEY (post_id) REFERENCES blog_posts(post_id), local_image_id SMALLINT, post_id INT, FOREIGN KEY (post_id) REFERENCES blog_posts(post_id) );" >> "create_schema.sql"
